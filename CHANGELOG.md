...
### Status
Engine built and dogfooding: **Shape · Teeth · Interview** done (`DESIGN.md` §10 steps 2–4) and the **lifecycle hooks** (§8) are wired via `extension.yml`. Remaining: **Prove** (one real build slice) → flip `mode: blocking` and tag 1.0.0, plus fleet glue.
