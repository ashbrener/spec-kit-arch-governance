...
- **citations_fresh** *(slice 006)* — **pinned** citations still match the cited artifact's current content state (a line-ending-normalized SHA-256 recorded in the per-repo sidecar `.spec-arch-pins.yml`). This closes the reverse-propagation gap: the five checks above test existence and status, never content movement. `derived_from` pins the upstream feature's `spec.md`; `cites` pins the full ADR file (an appended amendment registers as movement). Unpinned/orphaned/indeterminate states are advisory `note`s in every mode — only a *determinate* stale pin is failure-severity. Pins are written **only** by the explicit `repin --apply` verb (dry-run by default, writes only the invoking repo); the pin file's git history is the reconciliation audit trail.
...
3. **Teeth** — one validator, called from each repo's CI as a merge gate (advisory). ✅ *Done: `scripts/validate.py` — read-only, the ARCH-ADR-000 checks (five at 1.0.0; a sixth, `citations_fresh`, added by slice 006).*
...
6. **Then** — flip `mode: blocking` per repo and tag 1.0.0. ✅ *Done: blocking enforcement ships (`scripts/gate.py` + `before_implement` hook; the install ceremony refuses to flip a repo that isn't already clean — `guard_blocking_transition`, FR-006), advisory stays the default, and **`v1.0.0` is tagged**. Fleet-adoption glue landed as the self-configuring domain manifest + `sync` ([`specs/003-domain-manifest-sync`](./specs/003-domain-manifest-sync/)) — no external fleet manager required.*
