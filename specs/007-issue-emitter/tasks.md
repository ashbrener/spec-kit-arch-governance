...
- [X] T007 [US1] Implement in `scripts/issues.py`: `issues_plan(facts, mirrors)` (create / up-to-date rows for US1 scope), `render_title` / `render_body` (deterministic, R6), plan rendering per the CLI contract (`ISSUES PLAN — n row(s)` + `RESULT:` summary line). Make T006 pass.
...
- [X] T014 [US3] Implement resolve + reality-check adjustments in the apply loop (dismissed/record-only paths, one-note guarantee) and the adjusted report lines. Make T013 pass.
